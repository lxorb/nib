...
5. Publish the current release:
   ...
   ```
   gh workflow run publish-linux.yml --repo lxorb/nibeditor -f tag=v0.6.0
   ```
...
cd nib-bin && git add -A && git commit -m "nib 0.6.0" && git push
```
...
4. `gh workflow run publish-linux.yml --repo lxorb/nibeditor -f tag=v0.6.0` uploads
