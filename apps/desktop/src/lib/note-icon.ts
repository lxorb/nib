...
/** Writes the icon a note wears, or takes it away when `key` is null.
 *
 *  `key` is the library's own name for the icon, which is what the picker offers;
...
export async function setNoteIcon(path: string, key: string | null): Promise<void> {
  const before = await workspace.noteText(path)
  if (before === null) return

  const edit = frontMatterEdit(before, 'icon', key === null ? null : iconValue(key))
  if (!edit) return
