 ...
 *  prefix, the database and the media types out a second time, and
 *  asset-route.test.ts reads the file and holds the two to each other. */
 ...
 *  before the name was asked carry `image/jpg` and `image/svg`, which is why the
 *  name is asked first rather than second. */
export function assetType(path: string, stored?: string): string {
  return mimeOfPath(path) ?? stored ?? UNKNOWN_TYPE
}
